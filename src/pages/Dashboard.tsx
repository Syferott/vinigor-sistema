...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const pagoMes = db.contasPagar
      .filter(
        (c) =>
          c.status === 'paga' &&
          c.dataPagamento &&
          monthKey(c.dataPagamento) === mesAtual,
      )
    ...
    const aReceber = faturasAbertas.reduce(
      (s, f) => s + (f.total - valorPago(f.id)),
      0,
    )
    ...
    const contasAbertas = db.contasPagar.filter(
      (c) => c.status === 'pendente',
    )
  ...
  const funil = useMemo(() => {
    const count = (s: string) =>
      db.orcamentos.filter((o) => o.status === s).length
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="relative">
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (mostrarRecebido) {
                setMostrarRecebido(false)
                return
              }

              if (solicitarSenha()) {
                setMostrarRecebido(true)
              }
            }}
        ...
        <div className="relative">
          <KpiCard
            ...
            hint={
              stats.vencidasReceber > 0
                ? `${
                    mostrarAReceber
                      ? formatCents(stats.vencidasReceber)
                      : 'R$ ••••••'
                  } vencido`
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (mostrarAReceber) {
                setMostrarAReceber(false)
                return
              }

              if (solicitarSenha()) {
                setMostrarAReceber(true)
              }
            }}
            className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
            title={
        ...
        <div className="relative">
          <KpiCard
            ...
            value={
              mostrarAPagar
                ? formatCents(stats.aPagar)
                : 'R$ ••••••'
            }
            hint={
              `Despesas pagas no mês: ${
                mostrarAPagar
                  ? formatCents(stats.pagoMes)
                  : 'R$ ••••••'
              }`
            }
            tone="warn"
